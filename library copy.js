'use strict'
console.log('dingtalk---------load')
const user = module.parent.require('./user')
const meta = module.parent.require('./meta')
const db = module.parent.require('../database')
const passport = module.parent.require('passport')
const DingTalkStrategy = require('passport-dingtalk').Strategy
...
function PassportDingTalkStrategy(options, verify) {
  ...
  try {
    meta.settings.get('dingtalk-login', function (err, settings) {
      if (!err && settings.id && settings.secret) {
        passport.use('dingtalk', new DingTalkStrategy({
          clientID: settings.id,
          clientSecret: settings.secret,
          client: 'web',
          callbackURL: nconf.get('url') + '/dingtalk/login'
        }, function (accessToken, refreshToken, profile, done) {
          // req, accessToken, refreshToken, profile, expires, done
        ...
        strategies.push({
          name: 'wechat',
          url: '/dingtalk/loginqr',
          callbackURL: '/dingtalk/login',
          icon: 'fa-users',
          scope: '',
...
DingTalk.init = function (data, callback) {
  ...
  data.router.get('/api/dingtalk/getCode', async function(req, res) {
    let loginTmpCode = req.query.loginTmpCode
    ...
    let apiPath = `https://oapi.dingtalk.com/connect/oauth2/sns_authorize?appid=${settings.id}&response_type=code&scope=snsapi_login&state=2&redirect_uri=http://local.lcc.com/api/dingtalk/access&loginTmpCode=${loginTmpCode}`
    let resObj = await axios.get(apiPath)

    passport.use('dingtalk', new DingTalkStrategy({
      clientID: settings.id,
      clientSecret: settings.secret,
      client: 'web',
      callbackURL: nconf.get('url') + '/dingtalk/loginqr'
    }, function (accessToken, refreshToken, profile, done) {
      // req, accessToken, refreshToken, profile, expires, done
      console.log(accessToken, refreshToken, profile)
      return done(null, profile)
